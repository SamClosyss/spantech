...
class ir_ui_view(models.Model):
    ...
    def _postprocess_tag_button(self, node, name_manager, node_info):
        ...
        hide_button_ids = hide_button_obj.sudo().search([('access_management_id.company_ids','in',self.env.company.id),('model_id.model','=',name_manager.model._name),('access_management_id.active','=',True),('access_management_id.user_ids','in',request.env.uid)])
        company_ids = hide_button_ids.mapped('access_management_id.company_ids').ids
        ...
        if btn_store_model_nodes_ids:
            for btn in btn_store_model_nodes_ids:
                if btn.attribute_name == node.get('name'):
                    ...
                    # domain = "[('company_id','in',"+str(company_ids)+")]"
                    # node.attrib['attrs'] = "{'invisible':"+domain+"}"
                    break
                    # else:
                    #     hide = [btn]
                    #     break
    ...
    def _postprocess_tag_page(self, node, name_manager, node_info):
        ...
        hide_tab_ids = hide_tab_obj.sudo().search([('access_management_id.company_ids','in',self.env.company.id),('model_id.model','=',name_manager.model._name),('access_management_id.active','=',True),('access_management_id.user_ids','in',request.env.uid)])
        # translation_obj = self.env['ir.translation']
        # Filtered with same env user and current model
    ...
    def _postprocess_tag_div(self, node, name_manager, node_info):
        ...
        if name_manager.model._name == 'res.config.settings' and node.tag == 'div' and node.get('string'):
            for setting_tab in hide_button_obj.search([('access_management_id.company_ids','in',self.env.company.id),('model_id.model','=',name_manager.model._name),('access_management_id.active','=',True),('access_management_id.user_ids','in',request.env.uid)]).mapped('page_store_model_nodes_ids'):
