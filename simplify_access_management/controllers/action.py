...
class Action(Action):
    ...
    def run(self, action_id):
        ...
        actions_and_prints = []
        if res:
            for access in request.env['remove.action'].sudo().search([('access_management_id.company_ids','in',request.env.company.id),('access_management_id','in',request.env.user.access_management_ids.ids),('model_id.model','=',res.get('res_model'))]):
