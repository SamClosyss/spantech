...
class InternalCategory(models.Model):
    ...
    def _compute_sync_field(self):
        for rec in self:
            if not self.env.user.company_id.sync_with_pos_category:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            res = super(InternalCategory, self).create(vals_list)
            if vals.get('sync_with_pos_category') and vals.get('sync_with_pos_category'):
            ...
            elif 'sync_with_pos_category' not in vals and self.env.user.company_id.sync_with_pos_category:
                pos_category = self.env['merge.pos.category.wizard']
                # create pos category if sync boolean is true
                pos_category.create_pos_category(res.id)
        return res
    ...
    def write(self, vals):
        ...
        for rec in self:
            if rec.sync_with_pos_category and rec.pos_category_id:
                if vals.get('name'):
...
class Product(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if self.env.user.company_id.auto_sync_product_with_pos_category and vals.get('categ_id'):
