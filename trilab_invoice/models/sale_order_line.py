...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_invoiced(self):
        ...
        for line in self.filtered(lambda x: x.is_downpayment and x.order_id.x_use_ti):
            # For down payment sale.order.line count only qty_invoiced from down payment invoices
            qty_invoiced = 0.0
            for invoice_line in line.invoice_lines.filtered(lambda x: x.move_id._is_downpayment()):
